...
set(_exported_dependencies "cv_bridge;elevation_map_msgs;message_filters;rclpy;std_msgs;sensor_msgs;geometry_msgs;grid_map_msgs;tf2_ros")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to elevation_mapping_cupy_DEFINITIONS, elevation_mapping_cupy_INCLUDE_DIRS,
# elevation_mapping_cupy_LIBRARIES, and elevation_mapping_cupy_LINK_FLAGS.
# Additionally collect the direct dependency names in
# elevation_mapping_cupy_DEPENDENCIES as well as the recursive dependency names
# in elevation_mapping_cupy_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(elevation_mapping_cupy_DEPENDENCIES ${_exported_dependencies})
  set(elevation_mapping_cupy_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(elevation_mapping_cupy_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(elevation_mapping_cupy_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(elevation_mapping_cupy_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(elevation_mapping_cupy_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(elevation_mapping_cupy_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND elevation_mapping_cupy_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
